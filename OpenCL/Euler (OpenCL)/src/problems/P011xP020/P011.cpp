...
class P011: public Problem
{
	...
	private:
		...
		bool brute_setup()
		{
			...
			cl->init("src/problems/P011xP020/P011.cl","brute");
         cl->createReadWriteMemory(sizeof(int),&answer_mem);
         cl->createReadOnlyMemory(sizeof(int)*400,&digits_mem);
			cl->transferCPUObjectToGPU(sizeof(int),&answer_mem,&initialVal);
			cl->transferCPUObjectToGPU(sizeof(int)*400,&digits_mem,digits);
		...
		void makeString()
		{
			...
			for (int ii = 0; ii < 20; ii++)
				for (int jj = 0; jj < 20; jj++)
					digits[ii*20+jj] = ((int) s_arr[ii].at(jj * 3) - '0') * 10 + ((int) s_arr[ii].at(jj * 3 + 1) - '0');
		}
};
