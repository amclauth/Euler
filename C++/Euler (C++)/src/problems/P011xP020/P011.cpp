...
class P011: public Problem
{
   ...
   private:
      ...
      long bruteForce()
      {
         ...
         for (int ii = 0; ii < 20; ii++)
            for (int jj = 0; jj < 17; jj++)
            {
               ...
               {
                  cur = (int) (digits[ii][jj * 3 + kk * 3] - '0') * 10 + (int) (digits[ii][jj * 3 + 1 + kk * 3] - '0');
                  mult *= cur;
               }
               if (mult > max)
                  max = mult;
         ...
         for (int ii = 0; ii < 17; ii++)
            for (int jj = 0; jj < 20; jj++)
            {
               ...
               for (int kk = 0; kk < 4; kk++)
               {
                  cur = (int) (digits[ii + kk][jj * 3] - '0') * 10 + (int) (digits[ii][jj * 3 + 1] - '0');
                  mult *= cur;
               }
               if (mult > max)
                  max = mult;
         ...
         for (int ii = 0; ii < 17; ii++)
            for (int jj = 0; jj < 17; jj++)
            {
               ...
               for (int kk = 0; kk < 4; kk++)
               {
                  cur = (int) (digits[ii + kk][jj * 3 + kk * 3] - '0') * 10
                        + (int) (digits[ii + kk][jj * 3 + 1 + kk * 3] - '0');
                  mult *= cur;
               }
               if (mult > max)
                  max = mult;
      ...
      int optimizedBruteForce()
      {
         ...
         for (int ii = 0; ii < 20; ii++)
            for (int jj = 0; jj < 20; jj++)
               numbers[ii][jj] = ((int) digits[ii][jj * 3] - '0') * 10 + ((int) digits[ii][jj * 3 + 1] - '0');
      ...
      void makeString()
      {
         ...
         s_arr[18] = "20 73 35 29 78 31 90 01 74 31 49 71 00 86 81 16 23 57 05 54";
         s_arr[19] = "01 70 54 71 83 51 54 69 16 92 33 00 61 43 52 01 89 19 67 48";

         for (int ii = 0; ii < 20; ii++)
         {
            s_arr[ii].copy(digits[ii], s_arr[ii].size(), 0);
         }
      }
};
